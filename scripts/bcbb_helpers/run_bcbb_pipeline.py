...
def main(post_process_config_file, fc_dir, run_info_file=None, only_run=False,
         only_setup=False, ignore_casava=False, process_project=[], process_sample=[]):
    run_arguments = [[os.getcwd(),post_process_config_file,fc_dir,run_info_file]]
    if has_casava_output(fc_dir) and not ignore_casava:
        if not only_run:
            run_arguments = setup_analysis_directory_structure(post_process_config_file,
                            fc_dir, run_info_file, process_project, process_sample)
    else:
        if not only_run:
...
def run_analysis(work_dir, post_process, fc_dir, run_info):
    ...
    cmd = ["pm", "production", "run", project_to_run,
           "--sample", sample_to_run,
           "--flowcell", flowcell_to_run,
           "--drmaa",
           "--force"]
...
def setup_analysis(post_process_config, archive_dir, run_info_file):
    ...
    if run_info_file is not None and PROCESS_YAML:
        ...
        cl = [PROCESS_YAML_SCRIPT,
              run_info_backup,
              "--analysis", "Align_illumina",
              "--out_file",run_info_file,
              "--ascii","--clear_description"]
    ...
    if FC_SPECIFIC_AMPQ:
        ...
        assert machine_id and flowcell_position, \
                "Machine id and flowcell position could not be parsed from "\
                "run name '%s'" % run_name
        # write a dedicated post_process.yaml for the ampq queue
        if config.get("distributed",False):
            config["distributed"]["rabbitmq_vhost"] = \
                    "bionextgen-%s-%s" % (machine_id,flowcell_position)
        ...
        with open(post_process_config,"w") as fh:
            fh.write(yaml.safe_dump(config, default_flow_style=False,
                                    allow_unicode=True, width=1000))
    return [[os.getcwd(),post_process_config,archive_dir,run_info_file]]


def setup_analysis_directory_structure(post_process_config_file, fc_dir,
                                       custom_config_file, process_project=[],
                                       process_sample=[]):
    ...
    assert os.path.exists(fc_dir), \
            "ERROR: Flowcell directory %s does not exist" % fc_dir
    assert os.path.exists(analysis_dir), \
            "ERROR: Analysis top directory %s does not exist" % analysis_dir
    ...
    for project in fc_dir_structure.get('projects',[]):
        ...
        for sample_no, sample in enumerate(project.get('samples',[])):
            ...
            sample_files = do_rsync([os.path.join(src_sample_dir,f) for f in
                                    sample.get('files',[])],dst_sample_dir)
            ...
            for lane in sample_config:
                if 'multiplex' in lane:
                    for sample in lane['multiplex']:
                        sample['files'] = sorted([os.path.basename(f) for f in
                                          sample_files if f.find("_%s_L00%d_" %
                                          (sample['sequence'],int(lane['lane']))) >= 0])
                else:
                    lane['files'] = sorted([os.path.basename(f) for f in
                                    sample_files if f.find("_L00%d_" % int(lane['lane'])) >= 0])
            sample_config = override_with_custom_config(sample_config,custom_config)
            arguments = _setup_config_files(dst_sample_dir,
                                            sample_config,
                                            post_process_config_file,
                                            fc_dir_structure['fc_dir'],
                                            sample_name,fc_date,fc_name)
...
def _setup_config_files(dst_dir,configs,post_process_config_file,fc_dir,sample_name="run",fc_date=None,fc_name=None):
    ...
    with open(config_file,'w') as fh:
        fh.write(yaml.safe_dump(config_data_structure, default_flow_style=False,
                                allow_unicode=True, width=1000))
    ...
    local_post_process['galaxy_config'] = bcbio.utils.add_full_path(
                                          local_post_process['galaxy_config'],
                                          os.path.abspath(os.path.dirname(post_process_config_file)))
    ...
    if 'distributed' in local_post_process and 'platform_args' in local_post_process['distributed']:
        ...
        local_post_process['distributed']['platform_args'] = \
            "%s -J %s -o %s -D %s" % (local_post_process['distributed']['platform_args'],
                                      sample_name, slurm_out, dst_dir)
    local_post_process_file = os.path.join(dst_dir,"%s-post_process.yaml" % sample_name)
    with open(local_post_process_file,'w') as fh:
        fh.write(yaml.safe_dump(local_post_process, default_flow_style=False,
                                allow_unicode=True, width=1000))
    ...
    with open(run_command_file,"w") as fh:
        fh.write(" ".join([os.path.basename(__file__), "--only-run",
                           os.path.basename(local_post_process_file),
                           os.path.join("..",os.path.basename(dst_dir)),
...
def report_to_gdocs(fc_dir, post_process_config_file):
    ...
    for i in xrange(REPORT_RETRIES):
        try:
            ...
            LOG.warn("Uploading failed for {} ('{}'), retrying.. "\
                     "({} retries left)".format(runid,str(e),str(REPORT_RETRIES-i-1)))
    if succeeded:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Wrapper script for bcbb pipeline. If given a .yaml configuration file, "\
    ...
    parser.add_argument("fcdir", action="store", default=None,
            help="Path to the archive run folder")
    parser.add_argument("custom_config", action="store", default=None,
            help="Path to a custom configuration file with lane or sample-specific options "\
                 "that will override the main configuration", nargs="?")
    parser.add_argument("-r", "--only-run", dest="only_run", action="store_true", default=False,
            help="Don't setup the analysis directory, just start the pipeline")
    parser.add_argument("-s", "--only-setup", dest="only_setup", action="store_true", default=False, 
			help="Setup the analysis directory but don't start the pipeline")
    parser.add_argument("-i", "--ignore-casava", dest="ignore_casava", action="store_true", default=False, 
			help="Ignore any Casava 1.8+ file structure and just assume the pre-casava pipeline setup")
    parser.add_argument("-g", "--no-google-report", dest="no_google_report", action="store_true", default=False, 
			help="Don't upload any demultiplex statistics to Google Docs")
    parser.add_argument("--process-project", dest="process_project", action="store", default=[], nargs='+', 
			help="Only setup and run analysis for the specified list of projects")
    parser.add_argument("--process-sample", dest="process_sample", action="store", default=[], nargs='+', 
			help="Only setup and run analysis for the specified list of samples")
    args = parser.parse_args()
    main(args.config, args.fcdir, args.custom_config, args.only_run,
         args.only_setup, args.ignore_casava, args.process_project, args.process_sample)
